...
// reads another user's data. Heads up: not cryptographic, just slicing
// the JWT's payload segment (the middle dot-separated chunk) gives a
// stable value that changes whenever the identity does, without
// pulling in a crypto dependency. Returns 'anon' when no token's present.
export function fingerprintFromToken(token) {
  if (!token) return 'anon'
  const parts = String(token).split('.')
  if (parts.length < 2) return 'anon'
  // 24 chars of the payload is plenty for collision resistance
  // across realistic numbers of users on one device.
