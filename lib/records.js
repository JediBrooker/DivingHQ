// Records: personal / club / federation bests per (height,
// dive_code, position).
//
// Factory exposes:
...
// so existing clients (ScoreboardView, profile pages) keep working
// unchanged.
  ...
  async function checkAndApplyRecords({ eventId, competitorId, roundNumber }) {
    ...
    try {
      ...
      const scopes = [
        ...
        // against the Oceania record book.
        ...(c.continent
          ? [{
              scope: "continental",
      ...
      for (const s of scopes) {
        ...
        // locked though, so two first-time checks can both pass
        // this read and race into the upsert below. The upsert's
