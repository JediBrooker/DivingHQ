...
module.exports = function createEmail({ pool }) {
  ...
  // Alert the platform operator when a payout TRANSFER FAILS. Success needs
  // no email — the Stripe transfer is automatic and the recipient sees it
  // in-app; a failure (e.g. onboarding lapsed, account restricted) is the
  // only thing that wants a human. Recipient is PLATFORM_OPS_EMAIL (falls
  // back to EMAIL_FROM). Fire-and-forget.
