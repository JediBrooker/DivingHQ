...
// set — every helper then no-ops silently. This is the documented dev-mode behaviour
...
module.exports = function createEmail({ pool }) {
  ...
  // the payer already saw on the Stripe Checkout page.
  ...
  // A federation or club requested a withdrawal. The platform operator
  // fulfils payouts by real-world bank transfer, so they MUST hear about
  // every request — without this email a payout request would sit invisible
  // until someone happened to poll the back-office. Recipient is
  // PLATFORM_OPS_EMAIL (falls back to EMAIL_FROM so a solo operator gets it
  // without extra config). Fire-and-forget like every helper here.
  async function sendPayoutRequestedEmail({ orgId = null, clubId = null, payouts = [] }) {
    if (!mailer || !payouts.length) return;
    try {
      const to = process.env.PLATFORM_OPS_EMAIL || fromAddress();
      if (!to) return;
      const who = orgId
      ...
      const lines = payouts
        .map((p) => `  - ${(p.amount_cents / 100).toFixed(2)} ${p.currency} (payout ${p.id})`)
        .join("\n");
      const subject = `Withdrawal requested — ${name}`;
      const text = `${name} (${orgId ? "federation" : "club"}) requested a withdrawal:\n\n${lines}\n\nFulfil the bank transfer, then mark it settled via the payout back-office (GET /api/admin/payouts, POST /api/admin/payouts/:id/settle).\n\nDivingHQ`;
      await mailer.sendMail({ from: fromAddress(), to, subject, text });
    } catch (err) {
      console.error("[Payout Request Notify Error]", err.message);
    }
  }
