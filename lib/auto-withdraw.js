...
const { createWithdrawal } = require("./payout-ledger");
const { recordAudit } = require("./audit");

const SWEEP_INTERVAL_MS = 60 * 60 * 1000; // hourly
const STARTUP_DELAY_MS = 30 * 1000;       // let the app finish booting first

// One pass over every auto-withdraw-enabled org and club. Exported so tests
// can run it directly without timers. Returns { payouts: n } for logging.
async function sweepOnce({ pool, logger = console, email = null }) {
  let booked = 0;

  const orgs = (await pool.query(
    `SELECT id, auto_withdraw_min_cents
       FROM organisations
      WHERE auto_withdraw_enabled
        AND payout_account_name IS NOT NULL AND payout_account_details IS NOT NULL`,
  )).rows;
  for (const o of orgs) {
    try {
      const payouts = await createWithdrawal(pool, {
        orgId: o.id,
        note: "auto-withdrawal",
        minCents: o.auto_withdraw_min_cents || 0,
        requireBalance: false,
      });
      if (payouts.length) {
        booked += payouts.length;
        recordAudit(pool, {
          org_id: o.id, entity_type: "payout", entity_id: payouts[0].id,
          action: "payout.auto_requested",
          metadata: { payouts: payouts.map((p) => ({ id: p.id, amount_cents: p.amount_cents, currency: p.currency })) },
        }).catch(() => {});
        email?.sendPayoutRequestedEmail({ orgId: o.id, payouts });
      }
    } catch (err) {
      logger.warn?.({ err: err.message, org: o.id }, "[auto-withdraw] org sweep failed");
    }
  }

  const clubs = (await pool.query(
    `SELECT id, org_id, auto_withdraw_min_cents
       FROM clubs
      WHERE auto_withdraw_enabled
        AND payout_account_name IS NOT NULL AND payout_account_details IS NOT NULL`,
  )).rows;
  for (const c of clubs) {
    try {
      const payouts = await createWithdrawal(pool, {
        clubId: c.id,
        note: "auto-withdrawal",
        minCents: c.auto_withdraw_min_cents || 0,
        requireBalance: false,
      });
      if (payouts.length) {
        booked += payouts.length;
        recordAudit(pool, {
          org_id: c.org_id, entity_type: "payout", entity_id: payouts[0].id,
          action: "payout.auto_requested",
          metadata: { club_id: c.id, payouts: payouts.map((p) => ({ id: p.id, amount_cents: p.amount_cents, currency: p.currency })) },
        }).catch(() => {});
        email?.sendPayoutRequestedEmail({ clubId: c.id, payouts });
      }
    } catch (err) {
      logger.warn?.({ err: err.message, club: c.id }, "[auto-withdraw] club sweep failed");
    }
  }
...
function start({ pool, logger = console, email = null }) {
  const run = () =>
    sweepOnce({ pool, logger, email })
      .then(({ payouts }) => {
