...
module.exports = async function submitDiveList({
  ...
  // but actorLocalTime claims it was on time — we record it AND
  // flag the rows for referee adjudication.
  actorLocalTime = null,
  lateReview = false,
}) {
  ...
  if (event.round_rules) {
    ...
    if (Number.isFinite(cap) && capRounds > 0) {
      ...
      if (sum > cap + 0.001) {
        const err = httpErr(
          400,
          `Total DD ${sum.toFixed(1)} for the first ${capRounds} round${capRounds === 1 ? "" : "s"} exceeds the ${cap.toFixed(1)} limit`,
        );
        err.violations = [`Voluntary DD limit: ${sum.toFixed(1)} > ${cap.toFixed(1)}`];
        throw err;
      }
    }
  }
