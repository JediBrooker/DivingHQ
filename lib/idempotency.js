...
// Module-level pool, set by createIdempotency. Same pattern as
// lib/live-state.js (the matching restart-survival cache layer).
let pool = null;

/**
 * Wire up the idempotency layer with a Postgres pool. Call once
 * on server boot, BEFORE attaching route or socket handlers.
 *
 * @param {{ pool: import('pg').Pool }} opts
 * @returns {object} { httpMiddleware, socketCheck, socketStore, hashPayload, isValidUuidV4 }
 */
function createIdempotency({ pool: poolInstance }) {
  if (!poolInstance) throw new Error("createIdempotency requires { pool }");
  pool = poolInstance;
  return {
    httpMiddleware,
    socketCheck,
 ...
 * `req.body.idempotency_key`. If neither is present it calls
 * next() and the handler runs uncached — this matches the
 * "online direct call" path for clients that don't use the
 * outbox.
 */
 ...
 *
 * Errors are logged, not thrown — socket emit shouldn't fail
 * because the cache layer hiccuped.
 */
