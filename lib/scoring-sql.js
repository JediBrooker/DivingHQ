...
//
// These three used to be copy-pasted into a dozen queries; a
// scoring-rule change could fork results between the scoreboard,
// records, venue boards, and seeding. The builders below are
// pure string composition (no DB access) so the exact SQL each
// call site sends is unit-testable — see test/scoring-sql.test.js.
//
...
// collapses one dive's judge rows without grouping by d.dd, or
// "d.dd" when d.dd is already a grouping column (records,
// dive-by-dive history).
...
// clauses) is appended AFTER the chain; entries may reference
// any canonical alias (e.g. cdl.partner_id). Appending keeps the
// core chain byte-stable — inner joins on s/u-style keys commute
