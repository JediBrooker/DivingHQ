...
// — no DOM dependency, which keeps the importer dependency-free and
// the parsing unit-testable against saved HTML fixtures.
//
...
// parseCountryList() can refresh it from a live page if needed.
const COUNTRIES = [
...
// selectmeet.php — the page is a sequence of year sections:
...
// selectevent.php — many anchors to selectsheet.php?mref=N&eref=M.
...
// selectsheet.php — the ranked diver list for one event, plus the
// meet/event titles and full event date from the <h2>/<h3>.
function parseSheetList(html) {
  // Two <h2>s exist: a banner tagline and the content meet title.
...
// showsheet.php — one diver's full sheet. Header row defines the
// judge columns (J1..Jn), so we never hard-code the panel size.
