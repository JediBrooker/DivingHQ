...
function createStripe({
  ...
  // Expire a still-open Checkout Session on the connected account. Used to
  // retire a stale, under-priced pending session (e.g. one opened before a
  // late-fee deadline) so the payer can't complete it at the old price.
  // Best-effort: Stripe rejects expiring an already-completed/expired
  // session, so callers treat failures as non-fatal.
  async function expireCheckoutSession({ connectedAccountId, sessionId }) {
    assertEnabled();
    return client.checkout.sessions.expire(sessionId, { stripeAccount: connectedAccountId });
  }
