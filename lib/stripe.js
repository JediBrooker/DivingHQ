...
// 'au' (the platform's home) when unknown — Stripe's hosted onboarding
// confirms/corrects the real country anyway.
...
function createStripe({
  ...
  // Refuse the half-configured state outright: with a secret key but no
  // webhook secret, checkouts would happily CHARGE while every completion
  // webhook 400s — money taken, nothing ever fulfilled. Failing at boot is
  // the only safe behaviour; set both vars or neither.
  ...
  // expiring an already-completed/expired session — callers must then
  ...
  // to the currency's Stripe minor units; the returned refund's `amount` is
  // in STRIPE units — use fromStripeAmount() before storing it.
  ...
  // retried withdrawal safe — Stripe returns the original transfer, never a
  // second one.
