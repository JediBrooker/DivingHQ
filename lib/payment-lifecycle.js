...
//
// Several flows must kill a payer's still-open Stripe Checkout session before
// changing the thing it pays for (waiving a fine or charge, appealing a fine,
// cancelling/repricing a class enrolment, or replacing an abandoned checkout
// with a fresh one). Doing that naively loses money in two races this module
// exists to close:
//
...
//   zero rows; without a re-check the caller believes the checkout was
...
//   payment row is still 'pending', so marking it failed SUCCEEDS — and when
...
//   fix: when expire() fails, RETRIEVE the session — 'complete' means the
//   money is (or is about to be) captured, so report "paid" and leave the
//   row alone for the webhook.
//
...
//   'gone'        — the row already left 'pending' to a non-paid state
...
//                   "try again" message — guessing here is how money is lost.
...
async function retirePendingPayment({ pool, payments, logger }, p) {
  ...
  if (payments && payments.enabled && p.stripe_checkout_session) {
    try {
      ...
      // Ask Stripe which terminal state it reached before touching our row.
      try {
        ...
        // Can't reach Stripe at all — do NOT guess. Leaving the row pending
        // is the only state that can't lose money; the caller reports a
        // temporary failure and the action can simply be retried.
...
async function resumeOrRetireCheckout({ pool, payments, logger }, blocking) {
  ...
  if (payments && payments.enabled && blocking.stripe_checkout_session) {
    try {
      ...
      // they create next is a separate slot only if the old one left
...
// official accreditations — the payer got their money back, the 12-month
// grant goes with it).
