...
// `configured`, not `enabled`. Expiring or resuming a session that a payer
// already opened is cleanup on money that is halfway out the door, and it
// has to keep working after the payments flag is switched off. The fallback
// covers the test suite's fake payments objects, which have no `configured`.
...
async function retirePendingPayment({ pool, payments, logger }, p) {
  ...
  if (canReconcile(payments) && p.stripe_checkout_session) {
    try {
...
async function resumeOrRetireCheckout({ pool, payments, logger }, blocking) {
  ...
  if (canReconcile(payments) && blocking.stripe_checkout_session) {
    try {
