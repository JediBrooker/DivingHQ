...
//   }
//
// Heads up: sequence numbers reset on server restart (the bridge
// should re-sync from scratch via a one-shot fetch when it detects
// a sequence regression). Schema version bumps whenever the wire
...
// returns the LAST emitted sequence without advancing, since
// read-only snapshots must not perturb the socket-emit stream.
// Mixing the two modes is what kept tripping bridges into a
// phantom re-sync on every reconnect: HTTP boot would bump the
// counter, then the next socket emit would skip a number, and the
// bridge's "sequence regression" guard would treat it as a server
// restart.
...
async function buildScoreboardState({ pool, eventId, activePayload, onHoldReason = null, stamp = true }) {
  ...
  if (activePayload && activePayload.competitor_id) {
    ...
    if (dRes.rows.length) {
      ...
      // Dive total, present only when every judge has scored. The
      // full scoring pipeline (calc_event_dive_points) is in SQL;
      // we delegate to it so we don't drift from the rest of the
      // app's totals.
...
// try/catch so a build error in one event can't break the the
// score path or break other emits.
async function emitVenueState({ io, pool, eventId, activePayload, onHoldReason }) {
  ...
  // and this function fires on EVERY score submission, so paying
  // that cost on meets with no bridge in the room is pure waste.
  // The room name matches joinVenue in routes/socket.js; a bridge
  // always joins before its first snapshot (subscribe_venue), so
  // the bail can't starve a freshly-connected bridge. Callers are
  // fire-and-forget, none of them consume a return value.
