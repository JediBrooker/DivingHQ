...
// back to using server_now for both checks, same as the
// pre-outbox behaviour.
//
// A missing deadline (event has no entries_close_at) means theres
...
function evaluateDeadline({ deadline, actorLocalTime, serverNow, maxLateReviewMs = 24 * 60 * 60 * 1000 }) {
  ...
  if (!Number.isFinite(deadlineMs)) {
    // Malformed deadline, so be conservative and accept, just in
    // case (the strict gate above us would have rejected if it cared).
    return { verdict: 'ok' };
  }
  ...
  if (!Number.isFinite(actorMs)) {
    // Garbage timestamp from the client, so be conservative and use
