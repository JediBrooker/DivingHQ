...
//
// Two functions pulled out of routes/events.js once that file
// crossed the "agent-grep cost real tokens" line. Both are pure
// in the sense that they just take a Postgres client plus an
// event id and return rows, no closure dependencies on the
// events router. That's what makes them safe to share across:
//
...
// them in lib/ instead of routes/events/super-final-bridge.js
...
// Stable key for an unordered competitor pair, so a dive-off recorded
// as (A, B) matches a pair looked up as (B, A).
function diveOffPairKey(idA, idB) {
  return idA < idB ? `${idA}|${idB}` : `${idB}|${idA}`;
}

// Load resolved dive-offs (Appendix 3 §6) for an event into a
// Map(pairKey -> winner_id). Only rows with a winner count; when a
// pair was re-run, the most recently resolved row wins (rows are
// ordered oldest-first and the loop overwrites).
async function loadResolvedDiveOffs(client, eventId) {
  const r = await client.query(
    `SELECT competitor_a_id, competitor_b_id, winner_id
       FROM tiebreak_dive_offs
      WHERE event_id = $1 AND winner_id IS NOT NULL
      ORDER BY COALESCE(resolved_at, created_at) ASC`,
    [eventId],
  );
  const map = new Map();
  for (const d of r.rows) {
    map.set(diveOffPairKey(d.competitor_a_id, d.competitor_b_id), d.winner_id);
  }
  return map;
}

// Decide a pair's winner from the two totals, falling back to a
// resolved dive-off when the totals are equal (World Aquatics Art
// 4.1.5: equal totals are a tie, broken here by the referee's
// dive-off, Appendix 3 §6). Returns:
//   winner_id      the winner (null only when tied with no dive-off)
//   tied_on_total  true when the raw totals were equal
//   resolved_by    'total' | 'dive_off' | null
function resolvePairWinner(idA, totalA, idB, totalB, diveOffWinnerByPair) {
  if (totalA !== totalB) {
    return {
      winner_id:     totalA > totalB ? idA : idB,
      tied_on_total: false,
      resolved_by:   "total",
    };
  }
  const diveOffWinner = diveOffWinnerByPair?.get(diveOffPairKey(idA, idB)) || null;
  return {
    winner_id:     diveOffWinner,
    tied_on_total: true,
    resolved_by:   diveOffWinner ? "dive_off" : null,
  };
}

// Comparator for ordering an SF group's finalists: cumulative total
// descending, and on an equal total the recorded dive-off winner
// sorts ahead. Returns <0 when `a` should rank before `b`.
function compareSfFinalists(a, b, diveOffWinnerByPair) {
  if (b.cumulative_total !== a.cumulative_total) {
    return b.cumulative_total - a.cumulative_total;
  }
  const winner = diveOffWinnerByPair?.get(diveOffPairKey(a.competitor_id, b.competitor_id));
  if (winner === a.competitor_id) return -1;
  if (winner === b.competitor_id) return 1;
  return 0; // tied with no dive-off, caller decides whether to refuse
}
...
// competitor_a/competitor_b objects onto its wire shape, which is
...
async function loadH2hPairResults(client, h2hEventId) {
  ...
  // Resolved dive-offs (Appendix 3 §6) break a pair tied on total.
  const diveOffs = await loadResolvedDiveOffs(client, h2hEventId);
  const pairs = [];
  ...
  for (const g of [1, 2]) {
    ...
    for (let p = 0; p < 3; p++) {
      ...
      const { winner_id, tied_on_total, resolved_by } =
        resolvePairWinner(a.competitor_id, totalA, b.competitor_id, totalB, diveOffs);
      pairs.push({
        ...
        winner_id,
        tied:            winner_id == null,  // unresolved, still needs a dive-off
        tied_on_total,                        // the raw totals were equal
        resolved_by,                          // 'total' | 'dive_off' | null
      });
    }
  }
