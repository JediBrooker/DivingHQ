...
module.exports = function createMiddleware({ pool, JWT_SECRET, isMaintenance = () => false }) {
  ...
  // Back-compat shim, existing callers still expect a bare integer.
  ...
  function verifyToken(req, res, next) {
    ...
    jwt.verify(token, JWT_SECRET, { algorithms: ["HS256"] }, async (err, decoded) => {
      ...
      // Tokens minted before Migration 021 have no tv, so we accept
    ...
    verifyToken(req, res, async () => {
      try {
        ...
        const c = await pool.query(
          "SELECT id, org_id FROM clubs WHERE id = $1",
          [clubId],
        );
        ...
        const sameOrg = req.club.org_id === req.user.org_id;
        const orgRoles = req.user.org_roles || [];
        if (sameOrg && orgRoles.includes("org_admin")) return next();
        ...
        console.error("[requireClubAdmin]", err.message);
        res.status(500).json({ error: t(req, "errors.server_error") });
      }
    });
  };
  ...
  const requireClubAdminOnly = () => async (req, res, next) => {
    verifyToken(req, res, async () => {
      try {
        const clubId = req.params.id || req.params.clubId || req.body.clubId;
        if (!clubId) return res.status(400).json({ error: t(req, "errors.validation_failed") });
        ...
        console.error("[requireClubAdminOnly]", err.message);
        res.status(500).json({ error: t(req, "errors.server_error") });
      }
    });
  };
  ...
  function socketRequireRole(socket, roles = null) {
    ...
    // Maintenance lockdown. This is the single choke point every mutating
    // socket event passes through (submit_score, referee_*, meet_hold, …),
    // so refusing non-admins here covers the whole live-write surface with
    // one check. Sysadmins fall through and keep working, on purpose.
    if (maintenanceOn() && !socket.userIsSystemAdmin) {
