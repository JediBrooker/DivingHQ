...
// this module; the call-site shape `t(req, key, params)` lines up
// with i18next's `req.t(key, params)`, so the migration would stay local.
...
// resolution, it's just a Set membership check.
...
// is well under the cost of a single DB query, and avoids any any
// async resolution at request time. If a file is missing or
// malformed we just log and skip, the lookup falls back to English.
const DICTS = (() => {
  ...
  // to start if it can't be loaded: every other locale falls back
  // to it, so without en the helper is non-functional.
