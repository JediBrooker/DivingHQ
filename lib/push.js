...
//
// Both fall through to the `notifications` table, one audit trail
// per dispatched notification no matter which channel handled it.
//
// VAPID keys come from env (VAPID_PUBLIC_KEY / VAPID_PRIVATE_KEY /
// VAPID_SUBJECT). When VAPID_PUBLIC_KEY is unset the push side just
// no-ops, but the socket side still fires so the SPA inbox stays
// functional. Keeps dev boxes runnable without needing real Web
// Push setup.
...
module.exports = function createPushEngine({ pool, io }) {
  ...
  // Upsert a browser subscription. Same browser re-subscribing (say
  // after permission got revoked then regranted) hits the endpoint
  // UNIQUE and updates the existing row instead of creating a dupe.
  ...
  // Soft-delete so audit pointers stay stable for any prior
  ...
  // failed }. Never throws on a single sub failure, those land on
  // the row's failure_reason and the rest just keep going.
  async function sendNotification(userIds, payload) {
    ...
    // so different categories can carry different action sets
    // without any schema gymnastics.
    const fullData = { ...data };
    ...
    // Web Push sends get collected during the per-user pass below
    // and dispatched afterwards as one flattened (notification,
    // subscription) work list with bounded concurrency. The old
    // shape awaited every send serially, N users times M devices
    // of sequential push-service round-trips, which turned an
    // org-wide announcement to a few hundred users into minutes
    // of wall-clock time. Each job carries its own try/catch (same
    // per-subscription failure handling as before: 404/410 soft-
    // delete, last_used_at touch, error log) so one dead endpoint
    // still can't take down any other send.
    ...
    for (const uid of userIds) {
      // Insert the pending row first, just in case delivery fails
      // completely, so theres still an audit trail to retry from later.
      ...
      // ---- Channel 2: Web Push -----------------------------------
      if (!pushEnabled) {
        // No VAPID configured, so mark sent (socket counts as
        // delivered for an active session) and move on.
        await pool.query(
      ...
      if (!subs.rows.length) {
        // No registered devices, so it's socket-only delivery. Mark
        // sent so the row stays auditable, the SPA inbox will
        // surface it next time the user signs in.
        await pool.query(
          `UPDATE notifications SET status = 'sent', sent_at = now()
           WHERE id = $1`,
          [notificationId],
    ...
    // keeps a 500-user announcement down to seconds instead of
    // minutes without hammering any single push service. The jobs
    // never throw (each wraps its own try/catch), allSettled is
    // just belt-and-braces so a bug in one worker can't strand
    // the rest.
    if (pushJobs.length) {
    ...
    // when every subscription errored out.
  ...
  // the call after the row's already acknowledged is just a no-op.
  ...
  // rows, no point showing a sign-off request from yesterday.
