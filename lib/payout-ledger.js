...
async function createWithdrawal(pool, { orgId = null, clubId = null, note = null, minCents = 0, requireBalance = true }) {
  ...
  try {
    ...
    // authority (it rejects a not-active account), so this is UX gating —
    // don't book a payout that can't be sent.
...
// count against it). Never throws — returns the settled rows so the caller
// can report per-currency outcomes.
