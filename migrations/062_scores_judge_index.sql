...
--     — judge_id is the last key, not prefix-usable
--   * (event_id, round_number)
--   * (competitor_id)
-- so every analytics rollup sequential-scans the whole scores
-- table. The analytics endpoint fans out ~16 such rollups per
...
-- isn't usable here: the runner executes each file as one
