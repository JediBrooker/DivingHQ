...
-- verifier MUST NOT accept the second attempt of the same OTP.
--
...
-- (routes/auth.js) consume a code with a conditional UPDATE —
-- only a step strictly greater than the stored one is accepted —
-- so replays and concurrent presentations of the same code lose
-- the race and are rejected like any bad code.
--
-- NULL = no code consumed yet (or 2FA off / re-enrolled). The
-- 2FA setup/disable flows reset it alongside totp_secret.
-- Recovery codes are already single-use and aren't affected.
-- =============================================================
