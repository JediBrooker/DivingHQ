...
With **one event live** it's the familiar three-column board: the running **History** of completed dives on the left (click any card to open the **Score Correction modal**, with a live preview of trim sum + dive points + delta as you type); the active diver in the centre — name, country chip, dive code + DD + description, the live judge tile strip, a READY / DIVING / JUDGING status pill, the 60-second WA post-warning shot clock, a Failed / Cap / Re-dive referee row, and the bottom-pinned **Next Diver / Finalise** primary with an Auto-next picker; and **Standings** on the right with an **Announce** button that pushes the board to the spectator scoreboard.
